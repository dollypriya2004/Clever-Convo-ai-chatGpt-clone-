...
import {useRef,useEffect,useState} from 'react';/* componets add state variables to functional */
...
function App() {
  ...
  const [input, setInput]= useState('');/* state var and setter fun*/
  const { theme, toggleTheme } = useContext(ThemeContext);
  const [engines, setEngines] = useState([]); /* store list of engines*/
  const [selectedEngine, setSelectedEngine] = useState(null);/*track of currently selected engine*/
